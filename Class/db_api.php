...
if (!class_exists('db_api'))
{
	...
	class db_api
	{
		...
		public function connect()
		{
			...
			if (!$this->dbLink)
			{
				return array('error' => array('code'=>-100 , 'info'=>'数据库链接出错'));
			}else{
		...
		public function query($SQL , $Rdb = false , $echo_sql = false)
		{
			...
			if ($this->dbData)
			{
				...
				return array('error' => array('code'=>-110 , 'info'=>'错误的SQL语句'));
			}
		}
		...
		public function insert($tabName = null , $data = null , $is_unbuffered = false , $echo_sql = false)
		{
			if (!(is_array($data) && count($data) > 0))
				return array('error' => array('code'=>-110 , 'info'=>'错误的SQL语句'));
			//$this->echoErr('<b class="red">db->insert() 错误</b><div>请传递正确的数组</div>');
		...
		public function update($tabName = null , $data = null , $Cons = null , $is_unbuffered = false , $echo_sql = false)
		{
			if (!(is_array($data) && count($data) > 0))
				return array('error' => array('code'=>-110 , 'info'=>'错误的SQL语句'));
			//$this->echoErr('<b class="red">db->update() 错误</b><div>请传递正确的数组'.print_r($data).'</div>');
