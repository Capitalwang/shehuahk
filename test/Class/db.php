...
if (!class_exists('db'))
{
	...
	class db
	{
		...
		{
			try{mysql_close($this->dbLink);}catch(Exception $e){}
		}
		 ...
		 */
		public function select_to_2Array($SQL , $Key = false , $is_a_zH = false , $echo_sql = false)
		{
			...
			if ($this->dbData)
			{
				...
				if (mysql_num_rows($this->dbData) > 0)
				{
					if ($Key)
					{
						if ($is_a_zH)
						{
							while ($Q = mysql_fetch_assoc($this->dbData))
							{
								$Q[$Key] = $Q[$Key];
								$select[$Q[$Key]] = $Q;
							}
							...
							while ($Q = mysql_fetch_assoc($this->dbData))
							{
								$Q[$Key] = (is_numeric($Q[$Key])) ? $Q[$Key] * 1 : $Q[$Key];
								$select[$Q[$Key]] = $Q;
							}
						}
					}else{
