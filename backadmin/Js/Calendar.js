// JavaScript Document
document.write('<style type="text/css">fieldset{margin:5px;padding:0 12px 12px 12px;border:1px solid #D4D4D4;background-color:#F9F9F9;}legend{font-weight:bold;}</style>');
/**********************   日期选择  ***************************
*	@作者		: Enjoyd
...
//var WriteHead=1;
document.write('<iframe style="position:absolute;width:216px;display:none;" name="divCalendar" id="divCalendar" frameborder="0" scrolling="no"></iframe>')
/*头部信息CSS及JS函数*/
...
'#Tools {width:214px;background:#5563B0;padding:0px;}'+
'#Tools li{padding-bottom:1px;}'+
'#Tools a{color:#FFF;}'+
'#Tools a:hover{border:1px solid #2E3665;background:#979FCE;}'+
'#Title {width:214px;background:#CAD3E8;border-bottom:1px solid #000;}'+
...
if(Calendar_obj2){content+='<li style="width:42px;"><a href="javascript:parent.Calendar_GetDate('+year+','+month+',0);">To Month</a></li>'+
'<li style="width:43px;"><a href="javascript:parent.Calendar_GetDate('+date.getFullYear()+','+(date.getMonth()+1)+','+date.getDate()+');">今天</a></li>';}
else{content+='<li style="width:86px;"><a href="javascript:parent.Calendar_GetDate('+date.getFullYear()+','+(date.getMonth()+1)+','+date.getDate()+');">今天</a></li>';}
...
'</ul></div>'+
'<div id="Title" style="font-size:9px;"><ul><li style="color:#F00;">日</li><li>一</li><li>二</li><li>三</li><li>四</li><li>五</li><li style="color:#F00;">六</li></ul></div>'+
...
function Calendar(obj,obj2,id,TorF)
{
	var p=Calendar_obj = document.getElementById(obj) || obj;
	Calendar_obj2 = document.getElementById(obj2) || obj2;
	var re=/\d{4}-\d{1,2}-\d{1,2}/;
	var arr,Sdate;
	arr=re.exec(Calendar_obj.value);
	if(arr!=null){Sdate=Calendar_obj.value;}else{var mydate=new Date();
	Sdate=mydate.getFullYear()+'-'+(mydate.getMonth()+1)+'-'+mydate.getDate();}
	Sdate=Sdate.split("-");
	Calendar_Show(parseInt(Sdate[0],10),parseInt(Sdate[1],10),parseInt(Sdate[2],10));
	if (id) checkID = id;
	if (TorF == false) CalerTorF = false;
}
function Calendar_GetDate(y,m,d)
{
...
function Calendar_hide()
{
	...
	if (checkID) DateCheck(checkID,CalerTorF);
}
