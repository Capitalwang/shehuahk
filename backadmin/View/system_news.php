<?php
	class system_news extends start
	{
		...
		public function show()
		{
			...
			$list->SQL = "select id,title,taxis,uptime,is_view from system_news order by {$ORDER[$ors]}";
		...
		public function edit()
		{
			...
			if ($this->get['go'] == 'append')
			{
				...
				if (($edit = $this->db->select_to_1Array("select * from system_news where id='{$this->get['id']}'")) != false)
				{
		...
		public function del()
		{
			...
			$this->db->del('system_news','id in ' . $id);
		...
		private function append($url , $id = false)
		{
			...
			$arr['is_foot'] = ($post['is_foot'] == 'Y') ? 'Y' : 'N';
			$arr['time'] = mktime();
			$arr['uptime'] = mktime();
			
			if ($id)
			{
				$this->db->update('system_news' , $arr , "id='{$id}'");
			}else{
				$arr['id'] = $this->fun->rand_string(16);
				$this->db->insert('system_news' , $arr);
			}
