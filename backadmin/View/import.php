<?php
	...
	class import_csv
	{
		...
		private function get_cut($ch)
		{
			...
			elseif($ch == 'Very Good' or $ch == 'Good-Very Good')
			{
				$ch = 'VG';
			}
			elseif($ch == 'Good')
		...
		private function get_F($ch)
		{
			...
			elseif($ch == 'Slight' or $ch == 'Faint')
			{
				$ch = 'F/SL';
			}
			elseif($ch == 'Medium' or $ch == 'Medium Blue' or $ch == 'Medium Yellow' or $ch == 'Medium Green')
			{
				$ch = 'M';
			}
			elseif($ch == 'Strong' or $ch == 'Strong Blue')
			{
				$ch = 'S';
			}
			elseif($ch == 'Very Strong' or $ch == 'Very Strong Blue')
			{
				$ch = 'VS';
			}
			elseif($ch == 'None' or $ch == 'N')
			{
				$ch = 'N';
			}
			else
		...
		private function get_data($file , $val , $exchange , $BID , $is_new = true)
		{
			if (count($file))
			{
				...
				$array['cut'] = $this->get_cut($file[$val][5]);
				$array['buffing'] = $this->get_cut($file[$val][6]);
				$array['symmetry'] = $this->get_cut($file[$val][7]);
				$array['Fent_Isity'] = $this->get_F($file[$val][8]);
				...
				$file_4 = trim($file[$val][4]);
			    $file_3 = trim($file[$val][3]);
				$array['infml'] = sprintf('%.2f' , (float)$file[$val][12] * $exchange['USD']);
				$array['INTbid'] = @sprintf('%.2f' , $BID[$file_4][$file_3] * $exchange['USD']);
				
				$array['agio'] = @sprintf('%.2f' , $file[$val][12] / $array['weight'] / $BID[$file_4][$file_3] * 100 - 100);
		...
		public function exec($weight)
		{
			...
			if (is_file($this->file_address))
			{
				...
				//上架
				if(trim($file[$val][3]) != 'Color' or trim($file[$val][3]) !='')
				{
				...
				echo $weight ,' ct yes处理完成', '<br />';
			}else{
				echo $weight ,' ct no不存在,或者文件为空', '<br />';
			}
		}
	}
?>